...
public class AboutMe extends HibernateRubisHttpServlet {
	...
	private void listItems(User user, Session sess, ServletPrinter sp) {
		...
		// current sellings
		try {
			Query q = sess.createFilter(user.getItems(),
					"select this where this.endDate >= NOW()");
			lst = q.list();
		} catch (HibernateException he) {
			printError("Exception getting current sellings list: " + he
					+ "<br>", sp);
			return;
		}
		...
		try {
			Query q = sess
					.createFilter(user.getItems(),
							"select this where  TO_DAYS(this.endDate) > TO_DAYS(NOW()) - 30");
			lst = q.list();
		} catch (HibernateException he) {
			printError("Exception getting past sellings list: " + he + "<br>",
					sp);
			return;
		}
	...
	private void listBoughtItems(User user, Session sess, ServletPrinter sp) {
		...
		try {
			Query q = sess
					.createFilter(user.getBuys(),
							"select this.item, this.qty where TO_DAYS(this.date) >= TO_DAYS(NOW()) - 30");
			lst = q.list();
		} catch (HibernateException he) {
			printError("Exception getting bought items list: " + he + "<br>",
					sp);
			return;
		}
	...
	private void listWonItems(User user, Session sess, ServletPrinter sp) {
		...
		try {
			Query q = sess
					.createFilter(
							user.getBids(),
							"select this.item where TO_DAYS(this.item.endDate) > TO_DAYS(NOW()) - 30 group by this.item");
			lst = q.list();
		} catch (HibernateException he) {
	...
	private void listBids(User user, Session sess, ServletPrinter sp) {
		...
		try {
			Query q = sess
					.createFilter(
							user.getBids(),
							"select this.item, this.maxBid where this.item.endDate >= NOW() group by this.item");
			lst = q.list();
		} catch (HibernateException he) {
		...
		try {
			...
			printError("Exception getting items the user has bid on: " + e
					+ "<br>", sp);
			return;
		}
	...
	private void listRecommendations(User user, Session sess, ServletPrinter sp) {
		List lst = null;
		try {
			Query q = sess
					.createFilter(user.getItems(),
		...
		Iterator it = lst.iterator();
		sp.printHTML("<br>");
		sp.printRecommedHeader("Recommeded Items For You!");
		do {
			// Get the name of the items
			Item item = (Item) it.next();

			// display information about the item
			sp.printRecommed(item);
		} while (it.hasNext());
		sp.printItemFooter();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		result = result + "<h2>Information about " + user.getNickname()
				+ "<br></h2>";
		result = result + "Real life name : " + user.getFirstname() + " "
				+ user.getLastname() + "<br>";
		result = result + "Email address  : " + user.getEmail() + "<br>";
		result = result + "User since     : " + user.getCreationDate() + "<br>";
		result = result + "Current rating : <b>" + user.getRating()
				+ "</b><br>";

		SessionCount sc = new SessionCount();
		if (sc.get_a() < sc.get_peak()) {
			sp.printHTMLheader_for_about_me("RUBiS: About " + user.getNickname());
			sp.printHTML(result);
			...
			listRecommendations(user, sess, sp);
//			int sum=1;
//			for(int i=1;i<10;i++)
//				sum=sum+i;
//			System.out.println(sum);
			
		} else if (sc.get_a() < sc.get_peak()+sc.get_peakAfterCut()){
			sp.printHTMLheader_for_about_me("RUBiS: About " + user.getNickname());
			sp.printHTML(result);
			listBids(user, sess, sp);
			listItems(user, sess, sp);
